...
def cartesian_matrix(coords):
    # reate a distance matrix for the city coords that uses straight line distance
    matrix = {}
...
def read_coords(coord_file):
    # read the coordinates from file and return the distance matrix coords should be stored as comma separated floats,
...
def run_anneal(init_function, move_operator, objective_function, max_iterations, start_temp, alpha):
    ...
    iterations, score, best = anneal(init_function, move_operator, objective_function, max_iterations, start_temp,
                                     alpha)
    return iterations, score, best
...
def usage():
    print(
        "usage: python %s [-o <output image file>] [-v] [-m reversed_sections|swapped_cities] -n <max iterations>"
        " [-a hillclimb|anneal] [--cooling start_temp:alpha] <city file>" %
        sys.argv[0])
        ...
        if option == '-v':
            ...
            if arg == 'hillclimb':
                ...
                def run_anneal_with_temp(init_function, move_operator, objective_function, max_iterations):
                    return run_anneal(init_function, move_operator, objective_function, max_iterations, start_temp,
                                      alpha)
